...
batch_size=1
PREDICT_TFRECORD='./predict.tfrecords'
...
def read_and_decode_predict(filename):
    filename_queue = tf.train.string_input_producer([filename])
    reader = tf.TFRecordReader()
    _, serialized_example = reader.read(filename_queue)
    features = tf.parse_single_example(serialized_example,
    ...
    img = tf.decode_raw(features['img'], tf.uint8)
    img = tf.reshape(img, [1, input_h,input_w, 3])
...
if __name__ == '__main__':
    ...
    img_predict = read_and_decode_predict(PREDICT_TFRECORD)
    ...
    _2,logits_predict=inference(img_predict,False,True)
    '''
